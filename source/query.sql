-- D01 -- Jazdec, ktorý vyhral pretek na každom okruhu v Severnej Amerike počas sezóny 2021. 
    -- D01.1
with
...
-- D02 --  Jazdci, ktorí mali uzavreté kontrakty len s Ferrari a s nikým iným.
...
-- D03 --  Jazdec, ktorý vyhral 'domáci' pretek, teda pretek v krajine kde sa narodil. 
...
-- D04 -- Tímy, ktoré si konštruujú vlastný motor. 
SELECT DISTINCT tim.meno AS tim
    FROM motor_vyrobca
    JOIN tim on motor_vyrobca.vlastnik_tim = tim.id_tim;


-- D05 -- Režisér, ktorý mal na starosti každý pretek konajúci sa v Ázii 
...
-- D06 --  Jazdec s najlepšie plateným, už ukončeným kontraktom. 
...
-- D07 --  Kontrolný dotaz pre D1: Všetky preteky v Severnej Amerike v sezóne 2021 mínus preteky ktoré vyhral Max Verstappen (očakávaná prázdna množina) 
...
-- D08 -- Jazdci, ktorí mali alebo majú uzavreté kontrakty s talianskymi tímami 
SELECT DISTINCT jazdec.meno AS jazdec FROM KONTRAKT
    ...
    ORDER BY jazdec.meno ASC;


-- D09 -- Preteky, ktoré sa konajú na okruhu kratšom ako 5 kilometrov. 
SELECT DISTINCT pretek.nazov AS pretek, dlzka FROM 
    Pretek LEFT OUTER JOIN okruh ON prebieha_na = id_okruh
    WHERE dlzka < 5000;


-- D10 -- Výčet evidovaných jazdcov a krajín z ktorých pochádzajú, vrátane jazdcov bez krajiny a krajín bez jazdcov
...
-- D11 -- Zoznam pretekárov spolu s počtom pretekov, ktoré vyhrali za sezónu 2021. 
...
-- D12 -- Jazdec, pre ktorého neevidujeme žiaden trest. 
...
-- D13 -- Krajiny, ktoré vyprodukovali buď výherných jazdcov alebo výherné tímy
...
-- D14 -- Tímy, ktoré si vyrábajú vlastný motor a nevyhrali ani jeden pretek v sezóne 2021
...
-- D15 -- Jazdci, ktorí dostali počas sezóny 2021 v súčte tresty za vyše 1000€, zoradení podľa veľkosti trestu. 
...
-- D16 -- Pohľad, v ktorom budú všetci jazdci, počet výhier a počet trestných bodov. 
...
-- D17 -- Ktorý pretekár má najviac výhier a nedostal ani jeden trestný bod? (použi pohľad "vitazi")
...
-- D18 -- Po oneskorenom prešetrení bol udelený víťazovi 13. preteku sezóny 2022 časový trest piatich sekúnd a 10 000€. 
...
-- D19 -- Kvôli časovej penalte z oneskoreného trestu z D18 sa zmenil víťaz preteku, 
--          keďže druhý jazdec bol pozadu oproti víťazovi len tri sekundy. 
--          Novým víťazom trinásteho preteku sa stal Daniel Ricciardo, záznam o preteku musí byť upravený.
UPDATE Pretek
    SET vitaz = (SELECT id_jazdec FROM Jazdec WHERE meno = 'Daniel Ricciardo')
        WHERE id_pretek = (SELECT id_pretek FROM Pretek WHERE nazov = 'Heineken Dutch Grand Prix' AND sezona = 2021)
        AND sezona = 2021;


-- D20 -- Následkom nových regulácií prešli tímy na vyrábanie vlastných motorov / kupovanie od iných tímov, 
...
-- D21 -- Formula 1 je vraj šport pre mladých. Aké percento jazdcov bolo narodených pred rokom 1990? 
SELECT (
...
-- D22 -- Kto z jazdcov dostal najviac trestov? 
--          Zobraz so súčtom všetkých trestov vo všetkých štyroch kategóriách:
--          peňažné, časové, bodové a pozičné. 
SELECT meno,
...
-- D23 -- Vypíš všetky "nudné" preteky sezóny 2021. Ako nudný pretek si zadefinujeme akýkoľvek pretek, počas ktorého: 
...
-- D24 --  Formula 1 sa snaží expandovať mimo hranice Európy - vypíš všetky kontinenty a počet pretekov, ktoré sa na nich konali v roku 2021.
...
-- D25 -- Grafický tím si musí pripraviť vizuálne efekty pre prípad výhry ktoréhokoľvek pretekára na ktoromkoľvek okruhu, čo je veľa práce. 
--          Uľahčime im to vytvorením zoznamu všetkých možných okruhov a výhercov, usporiadané podľa mena jazdca. 
